...
const Hero = () => {
  ...
  return (
    <section 
      id="home" 
      className="relative min-h-screen flex items-center justify-center pt-24 pb-20 floating-particles" 
    ...
    >
      <div className="container mx-auto px-4 text-center relative z-10">
        ...
        <div className="max-w-4xl mx-auto mb-20">          
          ...
          <p className="text-sm sm:text-base md:text-xl text-muted-foreground mb-8 md:mb-12 max-w-3xl mx-auto leading-relaxed md:leading-[2.5] px-4" itemProp="description">
            {t('hero.subtitle')}
          </p>

          <div className="mb-6 md:mb-8 mt-8 md:mt-12">
            <span className="text-sm sm:text-lg md:text-2xl font-semibold inline-block px-3 md:px-4 py-2 border-[0.5px] border-white/30 rounded-full bg-gradient-to-r from-[#5DB5FF]/10 via-[#7B8EFF]/10 via-50% to-[#9B6FFF]/10 text-white shadow-[0_4px_12px_rgba(0,0,0,0.15)]">
              {t('hero.badge')}
            </span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row flex-wrap justify-center items-center gap-6 sm:gap-8 md:gap-16 text-sm text-muted-foreground mb-8 md:mb-12 mt-4 px-4">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div className="flex flex-col items-start">
                <span className="text-xs sm:text-sm">{t('hero.security')}</span>
                <span className="text-[10px] sm:text-xs opacity-70">{t('hero.securityEn')}</span>
              </div>
            </div>
            <div className="flex items-center gap-3 md:gap-4">
              <Zap className="w-4 h-4 text-muted-foreground scale-150 md:scale-[2]" strokeWidth={1} />
              <div className="flex flex-col items-start">
                <span className="text-xs sm:text-sm">{t('hero.latency')}</span>
                <span className="text-[10px] sm:text-xs opacity-70">{t('hero.latencyEn')}</span>
              </div>
            </div>
            <div className="flex items-center gap-3 md:gap-4">
              <Globe className="w-4 h-4 text-muted-foreground scale-150 md:scale-[2]" strokeWidth={1} />
              <div className="flex flex-col items-start">
                <span className="text-xs sm:text-sm">{t('hero.network')}</span>
                <span className="text-[10px] sm:text-xs opacity-70">{t('hero.networkEn')}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 md:gap-6 justify-center scale-100 sm:scale-110 md:scale-[1.3] mt-4 md:mt-6 px-4">
            <Button size="lg" className="bg-gradient-to-r from-[#5DB5FF] via-[#7B8EFF] via-50% to-[#9B6FFF] shadow-[0_4px_12px_rgba(0,0,0,0.15)] hover:opacity-90 hover-float hover:shadow-purple active:scale-105 active:shadow-[0_0_30px_rgba(255,255,255,0.9),0_4px_12px_rgba(0,0,0,0.15)] transition-all duration-300 w-full sm:w-48 text-white">
              <a href="/auth">{t('hero.startTrial')}</a>
            ...
            <Button variant="outline" size="lg" className="bg-gradient-to-b from-[#F5E6BB] to-[#FFB472] border-0 hover:opacity-90 hover-float hover:shadow-purple w-full sm:w-48 text-gray-900 hover:text-gray-900">
              <a href="/download">{t('hero.downloadClient')}</a>
            </Button>
          </div>
        </div>
